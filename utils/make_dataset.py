...
scaling_df = pd.read_csv('experiments/exp3/lambdas_1980_2013_.csv')
...
for path in paths:
    ...
    df['pred_tr_CI_upper'] = special.inv_boxcox(pred_CI_upper, lambdas.iloc[0].values)
    
    # Transformed lower confidence interval
    pred_CI_lower = df['pred0'] - 1.96 * np.sqrt(df['y_var0'])
    df['pred_tr_CI_lower'] = special.inv_boxcox(pred_CI_lower, lambdas.iloc[0].values)
    
    p = path.split('/')
    df.to_csv('experiments/exp3/tr_outputs_all/'+p[-1])
